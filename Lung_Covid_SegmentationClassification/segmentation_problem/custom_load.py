...
def LungDatasetLoader(path_to_data: str, batch_size: int, resize: Union[None, int] = None, workers: int = 4, pin_memory_device: Union[None, object] = None):
  ...
  def transform(image, mask):
    ...
    # Create a dataset
  data = LungDatasetSeg(path_to_data, transform=transform, resize_size=resize)
    
    # Create a DataLoader
  if "cuda" in pin_memory_device.type:
    data = DataLoader(dataset=data, batch_size=batch_size, num_workers=workers, shuffle=True, pin_memory=True, pin_memory_device=pin_memory_device.type)
  else:
    data = DataLoader(dataset=data, batch_size=batch_size, num_workers=workers, shuffle=True)

  return data

def LungDatasetLoaderTest(path_to_data: str, batch_size: int, resize: Union[None, int] = None, workers: int = 4, pin_memory_device: Union[None, object] = None):
  data = LungDatasetSeg(path_to_data, resize_size=resize)
