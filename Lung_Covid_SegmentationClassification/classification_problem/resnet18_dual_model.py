...
class ResNet18_dual(nn.Module):
    def __init__(self, img_channels: int, num_classes: int):
        super().__init__()
        self.model_images = ResNet(img_channels, num_classes, 18, BlockModel18_34)
        self.model_masks = ResNet(img_channels, num_classes, 18, BlockModel18_34)
    ...
    def forward(self, ximg, xmsk):
        ximg = self.model_images(ximg)
        xmsk = self.model_masks(xmsk)
        
        x = torch.cat((ximg, xmsk), dim=1)
